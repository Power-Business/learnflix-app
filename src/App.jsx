...
const SettingsPage = ({
  ...
  const handleEditClick = (video) => {
    ...
    setFormData({
      ...
      fileName: video.videoUrl.replace(/^\//, ""),
      isExternalUrl: video.videoUrl.startsWith("http"),
      thumbnail: video.thumbnail || "",
      assignedRoles: video.assignedRoles ? video.assignedRoles.join(", ") : "",
  ...
  const handleSubmit = (e) => {
    ...
    const videoData = {
      ...
      match: "New",
      views: editingVideoId
        ? videos.find((v) => v.id === editingVideoId)?.views || 0
        : 0,
            ...
            <div className="lg:col-span-1">
              <div className="bg-black/40 p-6 rounded-xl border border-zinc-800 shadow-xl sticky top-24">
                <h2 className="text-xl font-bold mb-6 flex items-center text-green-500">
                  {editingVideoId ? (
                    <>
                      <Edit className="w-5 h-5 mr-2" /> Edit Video
                    </>
                  ) : (
                    <>
                  ...
                  <div className="flex gap-2 pt-2">
                    {editingVideoId && (
                      <button
                        ...
                        className="w-1/3 bg-zinc-700 hover:bg-zinc-600 text-white font-bold py-3 rounded transition-colors"
                      >
                        Cancel
                      </button>
                    )}
                    <button
                      type="submit"
                      className={`flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded transition-colors flex items-center justify-center`}
                    >
                      <Save className="w-4 h-4 mr-2" />{" "}
                      {editingVideoId ? "Update Video" : "Add to Library"}
                    </button>
                  </div>
                </form>
              </div>
              ...
              <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
                ...
                <div className="space-y-3">
                  {videos.map((vid) => (
                    <div
                      key={vid.id}
                      className={`flex items-center justify-between bg-zinc-900 border p-4 rounded-lg group transition-colors ${
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="p-2 bg-zinc-800 text-zinc-400 hover:bg-blue-900/30 hover:text-blue-500 rounded transition-colors"
                          title="Edit Video"
                        >
...
export default function App() {
  ...
  const handleUpdateVideo = async (updatedVideoData) => {
    ...
    try {
      ...
      await setDoc(videoRef, updatedVideoData, { merge: true });
